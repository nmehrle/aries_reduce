...
def fitEllipGauss(data, edata=None, loc=None, retmod=False, maxfun=10000, xx=None, yy=None, disp=0):
    """2D data frame and uncertainties; fit 2D elliptical gaussian!"""
    from analysis import fmin, gaussian2d_ellip
    from phasecurves import errfunc
    
    if loc is not None:
        guess = [(data-np.median(data)).sum(), 0.6, 0.6, loc[0], loc[1], np.pi, np.median(data)]
    #mod0 = an.gaussian2d_ellip(guess, xx, yy)
    frameweights = 1./edata**2 #
    if not np.isfinite(data).all():
        goodind = np.isfinite(data) * np.isfinite(frameweights)
        fitargs = gaussian2d_ellip, xx[goodind], yy[goodind], data[goodind], frameweights[goodind]
    else:
        fitargs = gaussian2d_ellip, xx, yy, data, frameweights    
    cc = fmin(errfunc, guess, args=fitargs, maxfun=maxfun, disp=disp)
    mod = fitargs[0](cc, xx, yy)
    if retmod:
        ret = cc, mod
    else:
        ret = cc
